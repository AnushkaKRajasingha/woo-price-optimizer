...
class Plugin_menu extends Plugin_Core{
	...
	public function menuInit(){
		if (is_admin()) {						
			$admin_page = add_menu_page(  
					...
					WOOPO_PLUGIN_IMGDIR_URL.'/'.self::$current_plugin_data['PluginIcon']
					);
	...
	public function _options_page(){
		...
		global $p_set,$p_data;
		extract($page_param);
		include_once WOOPO_PLUGIN_PAGES.'/page-main.php';
	}
	...
	public function _option_page_sub(){
		if(is_admin() && isset($_REQUEST['page'])){
			...
			$filename = WOOPO_PLUGIN_PAGES.'/page'.$page_slug.'.php';
 			$page_param = array(
					'p_data' => self::$current_plugin_data,
			...
			if(file_exists($filename))
				...
				include_once WOOPO_PLUGIN_PAGES.'/page-404.php';
		}
	}
	...
	function _option_page_sub_v1(){
		if(is_admin() && isset($_REQUEST['page'])){
			...
			$filename = WOOPO_PLUGIN_PAGES.'/page'.$this->key_sub.'.php';
			$page_param = array(
